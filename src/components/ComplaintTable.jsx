...
export default function ComplaintTable() {
    ...
    const fetchComplaints = async () => {
        try {
            console.log('[ComplaintTable] Iniciando busca de registros...')
            console.log('[ComplaintTable] Supabase URL:', import.meta.env.VITE_SUPABASE_URL)
            ...
            if (error) {
                ...
                console.error('[ComplaintTable] Detalhes:', error.details)

                // Mensagens específicas por tipo de erro
                if (error.code === 'PGRST116') {
                    toast.error('Erro: Tabela não encontrada. Verifique o schema do banco.')
                ...
                console.log('[ComplaintTable] Primeiros registros:', data.slice(0, 2))
                setComplaints(data)
            ...
            console.error('[ComplaintTable] Stack:', error.stack)

            if (error.message === 'Timeout') {
                console.error('[ComplaintTable] Timeout após 30 segundos')
                toast.error('Timeout: A conexão com o Supabase está muito lenta. Verifique sua internet ou se o projeto está ativo.')
            } else if (error.message.includes('fetch')) {
                console.error('[ComplaintTable] Erro de rede')
                toast.error('Erro de rede: Não foi possível conectar ao Supabase. Verifique sua conexão.')
            } else {
        ...
        <>
            <div className="space-y-4 animate-fade-in">
                <div className="flex flex-col md:flex-row justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        <button onClick={exportExcel} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-green-700/20 text-green-400 border border-green-700/50 rounded-lg hover:bg-green-700/30 transition-all hover-lift">
                            <FileSpreadsheet size={18} /> <span className="hidden sm:inline">Excel</span>
                        </button>
                        <button onClick={exportPDF} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-red-700/20 text-red-400 border border-red-700/50 rounded-lg hover:bg-red-700/30 transition-all hover-lift">
